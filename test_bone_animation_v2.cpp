...
//"	vec4 localPosition = getBoneMatrix(boneIds[i])*vec4(pos,1.0);\n"
...
int main(int argc, char** argv)
{
	...
	//float* pixelData = new float(MAX_BONES * 16);
	std::vector<float> pixelData(MAX_BONES * 16, 0.f);
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, 4, MAX_BONES, 0, GL_RGBA, GL_FLOAT, NULL);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		for (int i = 0; i < nb_bones; i++) {
			...
			pixelData[16 * i + 0] = deform[0]; pixelData[16 * i + 1] = deform[3]; pixelData[16 * i + 2] = deform[6]; pixelData[16 * i + 3] = 0;
			pixelData[16 * i + 4] = deform[1]; pixelData[16 * i + 5] = deform[4]; pixelData[16 * i + 6] = deform[7]; pixelData[16 * i + 7] = 0;
			pixelData[16 * i + 8] = deform[2]; pixelData[16 * i + 9] = deform[5]; pixelData[16 * i +10] = deform[8]; pixelData[16 * i +11] = 0;
			pixelData[16 * i +12] = deform[9]; pixelData[16 * i +13] =deform[10]; pixelData[16 * i +14] =deform[11]; pixelData[16 * i +15] = 1;
			firstFrame = false;
		}
		
		//glActiveTexture(GL_TEXTURE1);
		glBindTexture(GL_TEXTURE_2D, boneMatrixTexture);
		glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 4, MAX_BONES, GL_RGBA, GL_FLOAT, &pixelData[0]);
